...
def parsing(worker,file):
	fileobj = open(file)
	print (fileobj)
	eventLogs.append(worker.line_counter(fileobj))
...
for line in array:
	...
	worker = xmlrpc.client.ServerProxy("http://"+line.rstrip('\n').split(';',1)[0], allow_none=True)
...
for file in glob.glob("*.*"):
	fileobj = open(file)
	# linecount = worker.line_counter(fileobj)
	# eventLogs.append(proxy[i%workers].line_counter(fileobj))
	# print (linecount)
